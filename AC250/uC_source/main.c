...
// Używane wyłącznie na żądanie (komenda CLEAR), nie podczas startu.
...
void inicjalizuj_parametry_modulu(void) {
  ...
  // --- INIT FLAG EXTERNAL EEPROM (First Boot) ---
#ifdef USE_EXTERNAL_EEPROM
  uint8_t init_flag = at24c512_read_byte(ADRES_EEPROM_INIT_FLAG);
  if (init_flag != 0xA5) {
    // Bez auto-wipe przy starcie (dla dużej mapy EEPROM trwa za długo i może
    // wyglądać jak brak startu). Pełne czyszczenie zostaje w komendzie CLEAR.
